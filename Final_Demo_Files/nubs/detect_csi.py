...
def signal_separation():
    print("2. Separating signal to get 5-50 Hz")

    eng = matlab.engine.start_matlab()
    eng.signal_separation(nargout=0)
    eng.quit()
    print("Signal separated.")
    

def main():

    print("1. Removing silence data...")
    matlab_file = './nubs/nubs_data_out/log_output.mat'
    silenced_data = get_silence_remove(matlab_file)
    print("Silence data removed.. ")
    
    filename = "./nubs/nubs_silence_out/silence.mat"
    data_mat = {'M': silenced_data}
    sio.savemat(filename, data_mat)

    signal_separation()

    if sys.argv[1] == "log1.dat":
    	separated_file = sio.loadmat("./nubs/SELECTIVE/sushant/separated_5_50/separated_3.mat")
    elif sys.argv[1] == "log2.dat":
        separated_file = sio.loadmat("./nubs/SELECTIVE/soham/separated_5_50/separated_1.mat")
    elif sys.argv[1] == "log3.dat":
        separated_file = sio.loadmat("./nubs/SELECTIVE/vin/separated_5_50/separated_1.mat")
    else:
        separated_file = sio.loadmat("./nubs/nubs_data_out/silence.mat")

    antenna = separated_file['M']
    ...
    clf = load("./nubs/CSI_MODEL_All.joblib")
    output = clf.predict(X)
    labels = ['Sushant', 'Soham', 'Vintony']
    a, c = mode(output) 

    prediction_name = labels[int(a[0])]
    print(prediction_name)

    cur_time = datetime.now().timestamp()

    data = json.dumps({
        'person': prediction_name,
        'time': str(int(cur_time))
    })

    try:
        http = urllib3.PoolManager()
        r = http.request('POST', 'http://localhost:5000/csi/new', headers={'Content-Type': 'application/json'}, body=data)
